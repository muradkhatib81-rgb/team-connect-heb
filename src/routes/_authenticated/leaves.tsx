...
function LeavesPage() {
    ...
    mutationFn: async (source: LeaveRequestRow) => {
      return submitFn({
        data: {
          ...
          note: i18n.t("leaves.cancelNoteDefault", {
            range: formatLeaveDateRange(source.start_date, source.end_date),
          }),
          kind: "cancellation",
    ...
    mutationFn: async (source: LeaveRequestRow) => {
      ...
      const { id } = await submitFn({
        data: {
          ...
          note: extendNote.trim() || i18n.t("leaves.extendNoteDefault", { date: extendEndDate }),
          kind: "extension",
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 p-4 md:p-6">
      <div className="page-title-row flex flex-wrap items-start justify-between gap-3 min-w-0">
        <div className="min-w-0">
          <h1 className="flex items-start gap-2 text-2xl font-semibold tracking-tight min-w-0 break-words">
            <Palmtree className="h-6 w-6 shrink-0 mt-0.5" />
            {i18n.t("leaves.title")}
          </h1>
