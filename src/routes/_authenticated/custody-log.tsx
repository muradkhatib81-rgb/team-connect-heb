...
function CustodyLogPage() {
  ...
  const todayLabel = formatAppDateTime(new Date(), { dateStyle: "full" }, i18n.language);

  return (
    <div className="space-y-6">
      <header className="page-title-row flex items-start gap-3 min-w-0">
        <div className="size-10 shrink-0 rounded-xl bg-teal-500/10 text-teal-600 flex items-center justify-center">
          <ClipboardList className="size-5" />
        </div>
        <div className="min-w-0">
          <h1 className="text-2xl font-bold break-words">{t("custody.logTitle")}</h1>
