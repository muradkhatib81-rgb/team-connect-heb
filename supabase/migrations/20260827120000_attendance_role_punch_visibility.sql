-- بصمة: تفعيل حسب دور النظام (رئيس قسم / نائب مدير / مدير فرع)
-- + إصلاح مطابقة المنصب حسب فرع الموظف
-- + إظهار واجهة البصمة عند السماح بالمنصب/الدور حتى بدون شفت اليوم
-- Does NOT touch user_roles / user_task_permissions tables.

CREATE TABLE IF NOT EXISTS public.attendance_role_punch_settings (
  role public.app_role PRIMARY KEY,
  can_punch boolean NOT NULL DEFAULT false,
  updated_at timestamptz NOT NULL DEFAULT now(),
  updated_by uuid NULL REFERENCES auth.users (id) ON DELETE SET NULL,
  CONSTRAINT attendance_role_punch_settings_allowed_roles CHECK (
    role IN (
...
INSERT INTO public.attendance_role_punch_settings (role, can_punch)
VALUES
  ('department_manager'::public.app_role, false),
  ('assistant_manager'::public.app_role, false),
  ('branch_manager'::public.app_role, false)
ON CONFLICT (role) DO NOTHING;

REVOKE ALL ON public.attendance_role_punch_settings FROM PUBLIC, anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.attendance_role_punch_settings TO authenticated;
GRANT ALL ON public.attendance_role_punch_settings TO service_role;

ALTER TABLE public.attendance_role_punch_settings ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS attendance_role_punch_owner ON public.attendance_role_punch_settings;
CREATE POLICY attendance_role_punch_owner ON public.attendance_role_punch_settings
  FOR ALL TO authenticated
  USING (public.is_platform_owner(auth.uid()))
  WITH CHECK (public.is_platform_owner(auth.uid()));

-- Allow authenticated to read (capabilities need it via SECURITY DEFINER anyway;
-- keep owner-only write via policy above — SELECT for all authenticated so UI can show)
DROP POLICY IF EXISTS attendance_role_punch_select ON public.attendance_role_punch_settings;
CREATE POLICY attendance_role_punch_select ON public.attendance_role_punch_settings
  FOR SELECT TO authenticated
  USING (true);

CREATE OR REPLACE FUNCTION public.attendance_role_allows_punch(_user_id uuid)
RETURNS boolean
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  RETURN EXISTS (
    ...
    JOIN public.attendance_role_punch_settings s ON s.role = ur.role
    WHERE ur.user_id = _user_id
      AND s.can_punch IS TRUE
  );
END;
$$;

REVOKE ALL ON FUNCTION public.attendance_role_allows_punch(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.attendance_role_allows_punch(uuid) TO authenticated, service_role;

-- Prefer job title row for the employee's branch (then global, then any).
CREATE OR REPLACE FUNCTION public.attendance_job_title_allows_punch(_user_id uuid)
RETURNS boolean
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  SELECT jt.can_punch_attendance
  INTO v_allowed
  ...
  ORDER BY
    CASE
      WHEN jt.branch_id IS NOT NULL AND jt.branch_id = p.branch_id THEN 0
      WHEN jt.branch_id IS NULL THEN 1
      ELSE 2
    END,
    jt.updated_at DESC NULLS LAST
  LIMIT 1;
...
AS $$
BEGIN
  RETURN public.attendance_job_title_allows_punch(_user_id)
    OR public.attendance_role_allows_punch(_user_id);
END;
$$;
...
BEGIN
  ...
  -- Show punch UI when feature+membership+title/role allow it (even if shift/leave blocks punch today).
...
-- Punch action: title OR system role may punch
...
BEGIN
  ...
  END IF;
  IF NOT public.attendance_user_allows_punch(v_uid) THEN
