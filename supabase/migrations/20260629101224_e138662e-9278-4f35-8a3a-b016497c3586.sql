...
DECLARE
  ...
  _arch_id uuid;
  _days numeric;
  _is_main_admin boolean;
BEGIN
  IF auth.uid() IS NULL THEN RAISE EXCEPTION 'לא מחובר'; END IF;
  _is_main_admin := public.has_role(auth.uid(), 'main_admin');
  IF NOT _is_main_admin THEN
  ...
  -- Main admin override: bypass the "must be inactive" requirement and the
  -- 30-day cooldown. Other admins were already rejected above.
