-- بصمة الدوام حسب المنصب (job_titles) — مش حسب الشخص.
-- Does NOT touch user_roles / user_task_permissions.

ALTER TABLE public.job_titles
  ADD COLUMN IF NOT EXISTS can_punch_attendance boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.job_titles.can_punch_attendance IS
  'When true, profiles with this job title may use attendance punch (when branch enabled + eligible).';

-- Existing titles: leave false so platform owner opts in (employees ON, managers OFF as configured).
-- New titles also default false until configured.
...
BEGIN
  ...
  -- Match profile.job_title → job_titles.name (same pattern as can_request_break).
  -- No matching title / empty title → no punch (must be configured by position).
...
REVOKE ALL ON FUNCTION public.attendance_job_title_allows_punch(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.attendance_job_title_allows_punch(uuid) TO authenticated, service_role;
...
BEGIN
  ...
  v_title_ok := public.attendance_job_title_allows_punch(v_uid);
...
-- Harden punch: also require job title allows punch
...
BEGIN
  ...
  END IF;
  IF NOT public.attendance_job_title_allows_punch(v_uid) THEN
