...
CREATE OR REPLACE FUNCTION public.list_attendance_report_departments(
  _branch_id uuid,
  _company_id uuid
)
RETURNS jsonb
...
REVOKE ALL ON FUNCTION public.list_attendance_report_departments(uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.list_attendance_report_departments(uuid, uuid) TO authenticated, service_role;
...
BEGIN
  ...
  combined AS (
    ...
    WHERE NOT EXISTS (SELECT 1 FROM scoped_profiles sp WHERE sp.id = h.user_id)
      AND v_filter <> 'one'
      -- Department filter is current assignment; do not pull punchers from other depts.
      AND _department_id IS NULL
  ),
  filtered AS (
